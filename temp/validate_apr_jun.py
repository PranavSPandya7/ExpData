"""
Temp script: run 13_atmo_lys_validate logic on Apr_Jun file
"""
import warnings; warnings.filterwarnings('ignore')
import base64, io
from datetime import datetime
from pathlib import Path
import sys; sys.path.insert(0, str(Path(__file__).resolve().parent.parent / 'scripts'))
...
CSV_IN   = Path(r"C:\Users\pandya\Documents\Github\docker\ExpData\temp\ExperimentData_20Apr_Jun.csv")
...
# Column name mapping: Apr_Jun file uses merged naming (atmotube_left__atmotube_temperature etc.)
PLOT_SIGNALS = [
...
def make_phase_plot(pid_str, df_pid):
    ...
    fig.suptitle(f'{pid_str} - Phase-Aligned Atmotube & LYS (Apr_Jun)',fontsize=11,fontweight='bold',y=0.98)
    ...
    for row,(pre,csfx,label,unit) in enumerate(PLOT_SIGNALS):
        ...
        if 'LYS' in pre:
            # LYS columns in Apr_Jun use merged naming: LYS1__lys_lux
            pass  # column name is already correct
        for col,ph in enumerate(PHASES):
...
def generate_html():
    print('  [validate_apr_jun] ...')
    df = pd.read_csv(CSV_IN, low_memory=False)
    if df['ParticipantID'].dtype in ('int64','float64'):
        df['ParticipantID'] = 'P' + df['ParticipantID'].astype(int).astype(str)
    ...
    html = [f'''<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Atmotube & LYS Quality Report - Apr_Jun</title>
<style>
...
<div class="header"><h1>Atmotube &amp; LYS - Quality Report (Apr_Jun)</h1><p>Generated: {datetime.now().strftime("%Y-%m-%d %H:%M")}</p></div>''']
    for pid in pids:
        ...
        for pre,csfx,label,_ in PLOT_SIGNALS:
            ...
            vals = pd.to_numeric(pdf[cn], errors='coerce')
            nan_pct = vals.isna().mean()*100
            hv = vals.dropna()
            html.append(f'<tr><td>{label}</td><td>{len(vals)}</td><td>{status_badge(nan_pct)}</td>'
                        f'<td>{hv.mean():.2f}</td><td>{hv.min():.2f}</td><td>{hv.max():.2f}</td>'
                        f'<td>{hv.quantile(0.25):.2f}</td><td>{hv.quantile(0.75):.2f}</td></tr>')
        html.append('</table>')
        img_b64 = make_phase_plot(pid, pdf)
        html.append(f'<img class="plot-img" src="data:image/png;base64,{img_b64}" alt="{pid} phase plot">')
        html.append('</div>')
    html.append(f'<div class="footer">Report generated from Apr_Jun file</div></body></html>')
