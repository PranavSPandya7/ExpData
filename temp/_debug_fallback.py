...
d = pd.to_datetime(p13['Date'] + '-2025', format='%d-%b-%Y').strftime('%Y-%m-%d')
start = pd.Timestamp(d + ' ' + p13['BikeU_start'], tz='UTC').tz_convert('Europe/Brussels')
end = pd.Timestamp(d + ' ' + p13['BikeU_end'], tz='UTC').tz_convert('Europe/Brussels')
print('BikeU:', start, '->', end)
...
for av in avros:
    ...
    print(av.name + ': ' + str(n) + ' samples, HR non-NaN=' + str(n_hr_ok) + '/' + str(n))
    if n_hr_ok == 0:
        ...
        print('  Fallback (0.3): ' + str(len(pk['Peaks'])) + ' peaks')
        pk2 = nk.signal_findpeaks(cleaned, sampling_rate=int(round(sf)), relative_height_min=0.1)
        print('  Fallback (0.1): ' + str(len(pk2['Peaks'])) + ' peaks')
